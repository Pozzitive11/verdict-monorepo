...
export class UtilFunctions {
  ...
  static createNgbDate(
    ...
    if (date instanceof Date)
      return this.createNgbDateFromDate(date, defaultDate);
    else
      ...
      if (eq) {
        const conditionIsTrue = (value: { [key: string]: any }) => {
          if (Array.isArray(value[col]))
            return value[col].some((val: any) => val == eq);
          else if (isDictionaryFullModel(value[col]))
      ...
      if (le) {
        data = data.filter(value => {
          if (Array.isArray(value[col])) {
            if (
              value[col].length > 0 &&
              value[col].every((val: any) => isSupportedArrayType(val))
            )
    ...
    for (const filter of sortingFilters) {
      data.sort((a, b) => {
        ...
        if (aIsDictionary && bIsDictionary) {
          if (a[filter.col].Name > b[filter.col].Name)
