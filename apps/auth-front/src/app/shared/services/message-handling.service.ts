...
export class MessageHandlingService {
  ...
  sendError(error: string) {
    const errorDateTime = formatDate(
      new Date(),
      "dd.MM.yyyy HH:mm:ss",
      "uk-UA",
    );
    this.dataErrors.update(errors =>
      errors.concat([`${errorDateTime}: ${error}`]),
    );
  }
  ...
  async alertError(error: ErrorModel | AsyncErrorModel, msg = "Щось не так") {
