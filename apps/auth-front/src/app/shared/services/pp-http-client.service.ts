...
export class PpHttpClientService {
  ...
  getGlobalFilters(processing_type: string) {
    ...
    return this.http.get<GlobalFilters>(this.url + "/global_filters", {
      params,
    });
  }
  ...
  requestFile(end_file: string) {
    return this.http.get<Blob>(this.url + "/user_file/" + end_file, {
  ...
  prepareUpload(glFilters: GlobalFilters | null, processing_type: string) {
    ...
    return this.http.post<Blob>(this.url + "/prepare_upload", glFilters, {
      responseType: "blob" as "json",
      params,
    });
  }
  ...
  getHistory(dateFilter: {
    ...
    return this.http.get<{ [key: string]: any }[]>(this.url + "/history", {
      params,
    });
  }
  ...
  loadPaymentsReconciliation(fromDate: string | null, toDate: string | null) {
    ...
    return this.http.get<{ [key: string]: string | number }[]>(
      this.url + "/c1_reconciliation/cashbox_to_logs",
      {
        params,
      },
    );
  ...
  loadCreditsReconciliation(fromDate: string | null, toDate: string | null) {
    const params = new HttpParams()
      ...
      .set("end_date", toDate ? toDate : "");
    return this.http.get<{ [key: string]: string | number }[]>(
      this.url + "/c1_reconciliation/by_credits",
      { params },
    );
  }

  loadBankToDatabaseReconciliation(
    ...
    return this.http.get<{ [key: string]: string | number }[]>(
      this.url + "/c1_reconciliation/bank_info_to_database",
      {
        params,
      },
    );
  ...
  splitRowK1(rowId: string) {
    return this.http.get<ProcessInfoModel>(
      this.url + `/rerun_register_reconciliation/${rowId}`,
    );
  }
  ...
  uploadPaymentDocs(
    ...
    return this.http.post<any[]>(this.url + "/payments_docs/" + rowId, files, {
      params,
    });
  }
  ...
  getContractPaymentsVerification(
    ...
    if (startDate)
      params = params.set(
        "start_date",
        UtilFunctions.formatNgbDate(startDate, "%Y-%m-%d"),
      );
    if (endDate)
      params = params.set(
        "end_date",
        UtilFunctions.formatNgbDate(endDate, "%Y-%m-%d"),
      );

    return this.http.get<any[]>(this.urlVerification + "/contract_payments", {
      params,
    });
  }

  getPaymentDocsVerification(
    startDate: NgbDate | null,
    endDate: NgbDate | null,
  ) {
    let params = new HttpParams();
    ...
    return this.http.get<any[]>(this.urlVerification + "/payment_docs", {
      params,
    });
  }
  ...
  getSchedulePayDetails(
    ...
    return this.http.get<Blob>(
      ...
      {
        responseType: "blob" as "json",
        params: this.fillVerificationParams(
          periods,
          project,
          rNumber,
          onlyFactoring,
  ...
  private fillVerificationParams(
    ...
    for (const period of periods)
      params = params.append(
        "periods",
        UtilFunctions.formatDate(period, false, "%Y-%m-%d"),
      );
